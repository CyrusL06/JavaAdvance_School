...
public class SwingDBDemo extends JFrame implements ItemListener,ActionListener {
	...
	//WHy ArrayList because we dont know how many customer before 
	...
	try {
			//Skips us to make a big Ass Query
	    ...
	    for (int i = 0; i < allCustomers.size(); i++) //goOverAllcustomer
	  	{              
	    	//1. Customerbutton arraylist
	    	//2. create newJbutton using customer name gettng all of them
	    	//Looping
...
///	    	JRadioButton.add(new JRadioButton())
	    	...
	    	//get the button and set the alignment of the components to the..
	    	customerbutton.get(i).setAlignmentX(Component.LEFT_ALIGNMENT);
	    	//inCustomerGroup we add this
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		else //if not
		{
			...
			//Enum 
			//In order to get the customerGroup in a button group
			//You need too use AbstractButton -> button added to buttonGroup 
			
			//if you dont like this code the code below, you can always use the customerButton ArrayList
			//ArrayLust declared above
			...
			//Which customer is selected
		     ...
		     try 
		     {
				...
				quantity = Integer.parseInt(inputNumber.getText()); //Integer is wrapper
